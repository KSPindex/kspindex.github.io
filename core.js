...
const Save = {
  ...
  def: {
    // Currency
    coins: 50, gems: 0, totalCoins: 50, totalGems: 0, _coinUpdatedAt: 0,
    // Player stats
    playerName: '레인저',
  ...
  _normalize(data) {
    ...
    this.d._coinUpdatedAt = Number(this.d._coinUpdatedAt || this.d.coinUpdatedAt || 0);
  ...
  addCoins(n) { if (!this.d) this.load(); const a=Math.floor(Number(n)||0); this.d.coins = Math.max(0, (Number(this.d.coins)||0) + a); this.d.totalCoins = Math.max(Number(this.d.totalCoins)||0, this.d.coins) + Math.max(0,a); this.d._coinUpdatedAt = Date.now(); this.save(); },
  ...
  resetAll() { const t = Date.now(); this.d = { ...this.def, _updatedAt: t, _coinUpdatedAt: t }; this.save(false); },
};
...
const ItemDB = {
  // Consumables
  potion_hp: { name: '응급 회복 키트', type: 'consumable', icon: '❤️', desc: 'HP 35 회복. 오염 현장에서 쓰는 최소한의 응급 처치 도구.', effect: 'heal', value: 35, price: 60 },
  potion_energy: { name: '휴대용 전력팩', type: 'consumable', icon: '⚡', desc: '에너지 55 회복. 장비를 잠시 더 움직이게 하는 보조 배터리.', effect: 'energy', value: 55, price: 55 },
  potion_water: { name: '정화수', type: 'consumable', icon: '💧', desc: '물 35 회복. 아직 완전히 안전하다고 말할 수 없는 귀한 물.', effect: 'water', value: 35, price: 50 },
  seed_pack: { name: '복원 종자 팩', type: 'consumable', icon: '🌱', desc: '종자 6개 획득. 살아남은 생태계의 마지막 표본.', effect: 'seeds', value: 6, price: 45 },
  shield_orb: { name: '방호막 캡슐', type: 'consumable', icon: '🛡️', desc: '짧은 시간 방호막 전개. 추락 폐기물과 독성 접촉을 버틴다.', effect: 'shield', value: 360, price: 160 },
  // Monster drops
  ...
  eco_blade: { name: '에코 블레이드', type: 'weapon', icon: '⚔️', desc: '재활용 금속 검. 확실하지만 과신은 금물.', dmg: 13, range: 1.35, speed: 1.1, price: 300 },
  water_gun: { name: '정화 물총', type: 'weapon', icon: '🔫', desc: '원거리 정화 압축수 발사기. 안정적이지만 화력은 낮다.', dmg: 8, range: 2.8, speed: 1.0, price: 200 },
  thorn_whip: { name: '가시 채찍', type: 'weapon', icon: '🌿', desc: '넓은 범위의 제압용 생체 도구.', dmg: 10, range: 2.3, speed: 0.85, price: 350 },
  thunder_hammer: { name: '천둥 망치', type: 'weapon', icon: '🔨', desc: '강력하지만 무거운 근접 장비.', dmg: 28, range: 1, speed: 0.45, price: 500 },
  crystal_staff: { name: '수정 지팡이', type: 'weapon', icon: '🔮', desc: '오염 결정의 공명을 끊는 장거리 장비.', dmg: 22, range: 3.5, speed: 0.75, price: 600 },
  // Tools
  broom: { name: '빗자루', type: 'tool', icon: '🧹', desc: '기본 청소 도구', power: 1.0, energy: 1.0, price: 0 },
  vacuum: { name: '에코 청소기', type: 'tool', icon: '🌀', desc: '현장용 청소기. 오염을 조금 더 빠르게 걷어낸다.', power: 1.2, energy: 0.95, price: 300 },
  nano: { name: '나노 군단', type: 'tool', icon: '✨', desc: '고성능 복원 장비. 넓은 정화에는 여전히 에너지 관리가 필요하다.', power: 1.55, energy: 0.85, price: 600 },
...
const StoryDialogues = {
  ...
  intro: [
    { speaker: '???', portrait: 'elder',
      text: '...여기까지 왔구나.' },
    { speaker: '비상방송', portrait: 'elder',
      text: '난 이 땅의 마지막 수호자, 비상방송이다.' },
    { speaker: '비상방송', portrait: 'elder',
      text: '한때 이곳은 푸르른 초원과 맑은 강이 흐르는 아름다운 세계였지...' },
    { speaker: '비상방송', portrait: 'elder',
      text: '하지만 인류가 편리함만을 좇은 대가로, 세계는 오염에 잠식되었다.' },
    { speaker: '비상방송', portrait: 'elder',
      text: '공장의 매연은 하늘을 낮게 눌렀고, 버려진 플라스틱과 폐기물은 비처럼 쏟아져 토양을 덮었다.' },
    { speaker: '비상방송', portrait: 'elder',
    ...
    { speaker: '비상방송', portrait: 'elder',
      text: '셋째, 하늘에서 떨어지는 폐기물을 방치하지 마라. 그것은 땅에 닿는 순간 다시 오염이 된다.' },
    { speaker: '비상방송', portrait: 'elder',
      text: '각 구역의 오염원, 폐기물 낙하, 변이 생물을 충분히 억제해야만 보스 오염체가 모습을 드러낼 것이다.' },
    { speaker: '비상방송', portrait: 'elder',
  ...
  monster_guide: [
    ...
    { speaker: '비상방송', portrait: 'elder',
      text: '이 생물들은 오염의 결과다. 처치보다 중요한 것은, 더 이상 이런 것들이 태어나지 않는 땅을 만드는 일이다.' },
  ],
  ...
  boss_intro: [
    { speaker: '비상방송', portrait: 'elder',
      text: '느껴지나... 이 사악한 기운을...' },
    { speaker: '비상방송', portrait: 'elder',
      text: '대형 오염체가 모습을 드러냈다. 이건 단순한 전투가 아니라, 누적된 방치의 결과와 마주하는 순간이다!' },
    { speaker: '비상방송', portrait: 'elder',
  ...
  victory: [
    { speaker: '비상방송', portrait: 'elder',
      text: '...해냈구나. 하지만 이 승리는 끝이 아니라 복구의 시작이다.' },
    { speaker: '비상방송', portrait: 'elder',
